...
# net = cv2.dnn.readNet('/home/jsw/WagonNumber_Detection/YoloV3_Config/new/yolov3_training_10000.weights',
        ...
        with open('screen1.jpg', 'wb') as f:
            for chunk in vid.iter_content(chunk_size=1024):
                ...
                if a != -1 and b != -1:
                    ...
                    # img = cv2.frombuffer(np.fromstring(jpg, dtype=np.uint8), cv2.IMREAD_COLOR)
        ...
        # logger.info("Cam_1_alive-{}".format(datetime.datetime.now()))
                    ...
                    if len(idxs) > 0:
                        ...
                        for i in idxs.flatten():
                            print("Object detected-{}".format(classes[classIDs[i]]))
                            ...
                            if mapping != 1:
                                ...
                                if "code" == classes[classIDs[i]]:
                                    ...
                                    if img_crop.shape[1] != 0 and img_crop.shape[0] != 0:
                                        ...
                                        if codex:
                                            ...
                                            if len(codex):
                                                ...
                                                # Connecting SQL for insert data
                                                try:
                                                    print("Try to Connect SQL for Trigger")
                                                    logger.info("Try to connect mssql server")
                                    ...
                                    if "coupling" == classes[classIDs[i]]:
                                        couplingcount += 1
                                        if couplingcount == 1:
                                            print("Coupling has been detected Restart variable")
                                            mapping = 0
                                            mapping1 = 0
                                            allcode = []
                                            insertdata = []
